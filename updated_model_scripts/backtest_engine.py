...
class BacktestEngine:
    ...
    def run_simulation(self):
        console.print("[bold cyan]Running SOTA + Inventory Simulation (No Slippage)...[/]")
        
        # Defensive Check
        required = ['atr', 'macd_z', 'rsi']
        if not all(col in self.df.columns for col in required):
            console.print(f"[bold red]CRITICAL ERROR: Columns {required} missing.[/]")
            console.print("Please delete 'AMD_physics_data.csv' and run data_manager.py again.")
            return
        ...
        console.print(f"[yellow]Skipped {skipped_toxic} candles due to Toxic Time/Event Risk.[/]")
